...
export async function GET(req: Request) {
  try {
    ...
    const backendRes = await fetch(
      `${backendUrl}/slack/sync_batch/tick?uid=${uid}&agent_id=${agent_id}&batch_id=${encodeURIComponent(batch_id)}`,
      {
        method: "GET",
        headers: {
    ...
    console.error("Error in Slack batch sync tick API route:", error);
    return new NextResponse(
